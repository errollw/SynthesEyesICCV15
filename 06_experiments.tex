...
To analyze the effect of different head pose ranges we rendered an additional targeted dataset (\dataset targeted) for a typical laptop setting ($10^{\circ}$ pose variation, $20^{\circ}$ gaze variation).
...
These results suggest that both the \dataset~and UT datasets do not capture all variations present in the test set, while each dataset covers different aspect of the test data. 
For example we capture more variations in lighting conditions and facial appearances, while the UT dataset contains real eye movements captured from more subjects.
...
\commentY{Not sure this assumption is reasonable to justify this order of pre-training + fine tuning}
Assuming that the~\dataset~dataset has wider range of appearance variation, we first trained the same CNN model with the~\dataset~dataset, and further fine-tuned the model with the UT dataset.
This fine-tuning model achieved better performances on both settings (untargeted $\mu\!=\!11.12^{\circ}$, targeted $\mu\!=\!7.90^{\circ}$).
...
%As shown in \autoref{fig:gazeResult}, it is possible to achieve better performances (untargetted $\mu\!=\!11.12^{\circ}$, targeted $\mu\!=\!7.90^{\circ}$) when we train with both datasets (pre-train with \dataset, and then fine tune with UT), perhaps because they capture different degrees of variation.
