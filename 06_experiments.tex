...
While it is important to cover a wide range of head poses to handle arbitrary camera settings, this can make training more difficult.
...
\citet{fu2011neural}~proposed a method to use an unsupervised pre-training result on synthetic data for fine-tuning on real data, and \citet{peng2014exploring}~reported the invariance obtained from pre-training on synthetic data is beneficial for fine-tuning.
We therefore finally evaluated the performance by training and fine-tuning using both datasets (see~\autoref{fig:gazeResult}).
