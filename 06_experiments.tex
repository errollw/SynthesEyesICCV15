...
%
First, our system CLNF Synth ($\mathrm{Mdn}=0.0110$) trained on only ten participants in four lighting conditions results in very similar performance to a system trained on unconstrained in-the-wild images, CLNF 300-W ($\mathrm{Mdn}=0.0110$). This suggests the importance of high-quality consistent labels.
%
...
While the 300-W images represent challenging conditions for eyelid registration they do not feature iris labels and are not representitive of conditions encountered during everyday human-computer interaction.
...
\begin{figure}
    \centering
    \includegraphics[width=\columnwidth]{gazeResult_v2.pdf}
    \caption{Gaze estimation performance on MPIIGaze using different training datasets. Red bar is median, and blue line represents a practical lower-bound (within-dataset cross-validation score).\commentT{Is the lower bound median as well?}}
    \label{fig:gazeResult}
\end{figure}

%\commentY{Edited this paragraph}
To evaluate our method for appearance-based gaze estimation we performed the cross-dataset validation as described in~\citet{zhang15_cvpr}.
%where they trained and tested the model on different datasets. 
Specifically, we synthesized training images using the same camera settings as in the UT dataset~\cite{sugano2014learning} and used the same normalization scheme for the test data.
...
We then trained the same convolutional neural network (CNN) model as described in~\cite{zhang15_cvpr} on both synthetic datasets and evaluated their performance on MPIIGaze as the testing dataset.

\commentA{do we compare to the real or synthesised UT images here?}
...
in~\autoref{fig:gazeResult}, the CNN model trained on our generic~\dataset dataset achieved the same performance of 12.66$^{\circ}$ as the model trained on the UT dataset (12.67$^{\circ}$).
This confirms that our approach can synthesise data that is comparable to a real-world data collection as performed in~\cite{sugano2014learning} for the appearance-based gaze estimation task.
...
These results suggest that both the \dataset~and UT datasets still have error sources that cause this performance gap.
...
If the target setting can be preliminary specified, as the laptop gaze interaction case in \citet{zhang15_cvpr}, it is  possible to target the synthesis of training data to the expected head pose and gaze ranges.
...
To analyse the effect of different head pose ranges we rendered an additional targeted dataset for a typical laptop setting ($10^{\circ}$ pose variation, $20^{\circ}$ gaze variation).
...
For comparison, we also re-sampled a subset of the UT dataset as described in~\cite{zhang15_cvpr} that has the same gaze and head pose distribution as MPIIGaze.
...
To make both datasets more comparable, we further divided the UT subset into five groups with 10 subjects each.
...
Trained on our \dataset dataset the CNN even achieves a slight performance improvement over the UT dataset of 0.66$^{\circ}$
% $\textrm{Mdn}\!=\!8.95^{\circ}$ and $\textrm{Mdn}\!=\!8.29^{\circ}$ for UT and \dataset respectively.
...
\begin{figure}
    ...
    \caption{Per-eye-model gaze estimation performance on MPIIGaze. Red represents worst scores. Note how some eye-models have proved more useful than others. \commentT{Does it matter that it's left eye for some right eye for others in the small images?} }
...
Appearance-based gaze estimation performs best when trained and tested on the same person, as the training data includes the exact same eye-shape appearances that can be occur during testing.
