...
Eyes are important for the entertainment industry, who want to model them realistically across different characters and with potentially dramatic appearance (e.g. crying).
\mbox{\citet{ActiBlizEyes}} recently developed techniques for real-time modelling of eye wetness, redness, refraction, and ambient occlusion in a standard rasterization pipeline, as well as approximations for light-transfer properties of eyelids.
...
Aside from visual effects, previous work has used computer models to examine the eye from a medical perspective.
\citet{sagar1994virtual} built a detailed model of the eye and surrounding face in a virtual environment that mechanically simulated surgery with finite element analysis.
