...
\commentY{swapped the section order, I think this is more important point than landmark annotation}
...
For the eye shape registration task, we need an additional ground-truth annotation of eye-region landmarks in the training images.
...
For eye-region registration \commentY{how about gaze estimation?}, we rendered a generic \dataset dataset of 11,382 images covering $40^{\circ}$ of viewpoint (i.e. head pose) variation and $90^{\circ}$ of gaze variation.
...
\commentY{This claim is a little misleading. We practically need to consider the model construction time too; the rendering time can be much shorter with the simple UT dataset models}
...
\commentY{As I commented before, this is not directly related to the model/data itself. I think it's better to simply mention in the experiments section as an additional analysis. This adapted rendering becomes the contribution if we can adjust, e.g., lighting condition to the target domain.}
